...
THEMES = [
    ...
    T("hologram", "Hologram", "tech", "Blue projection with slice glitches and a scan band.",
      ...
      light=dict(bg="#eef8ff", bg2="#d2e8fa", ink="#06243d", muted="#3f6d8f", acc="#0a78c2", acc2="#0aa0d8"),
        hero=dict(type="glow", font="sans", ls=1,
                  back=[("hexgrid", dict(size=20, op=0.14)),
                        ("glow_spot", dict(cx=450, cy=150, r=320, op=0.6)),
                        ("light_shafts", dict(n=5, op=0.2)),
                        ("scanlines", dict(op=0.12))],
                  front=[("flare", dict(yy=214)), ("glitch_bars", dict(n=9))],
                  device=dict(light=("rail", dict(side="left", w=14)))),
    ...
    T("radar", "Radar", "tech", "A sweep finds four contacts. Green on near-black.",
      ...
      light=dict(bg="#eff7f1", bg2="#d6e8db", ink="#072415", muted="#3a6b4f", acc="#0d7a45", acc2="#0f9f5b"),
        hero=dict(type="plain", font="mono", name_size=48, reserve=330,
                  back=[("hexgrid", dict(size=24, op=0.07)),
                        ("speckle", dict(n=140, op=0.12)),
                        ("ticks", dict(every=30, y=8)),
                        ("ticks", dict(every=30, y=226)),
                        ("radar", dict(cx=728, cy=120, rad=150)),
                        ("beacon", dict(cx=250, cy=196, dur=5))]),
    ...
    T("signal", "Signal", "tech", "An oscilloscope trace rolling under the name.",
      ...
      light=dict(bg="#f5f7fb", bg2="#e0e6f2", ink="#0c1a33", muted="#4c5f80", acc="#b3720b", acc2="#2f5fd0"),
        hero=dict(type="plain", font="mono", name_size=50,
                  back=[("hexgrid", dict(size=20, op=0.12)),
                        ("ticks", dict(every=30, y=8)),
                        ("ticks", dict(every=30, y=228)),
                        ("beacon", dict(cx=812, cy=52, dur=4))],
                  front=[("oscilloscope", dict(yy=196, amp=22, period=96)),
                         ("oscilloscope", dict(yy=204, amp=9, period=58,
                                               dark=dict(colour="#7aa2ff"),
                                               light=dict(colour="#2f5fd0")))]),
    ...
    T("pixel", "Pixel", "tech", "Eight-bit skies, a block floor, a blinking star field.",
      ...
      light=dict(bg="#f4f4ff", bg2="#d8dcf5", ink="#14143a", muted="#55558a", acc="#a86600", acc2="#0a8fd0"),
        hero=dict(type="plain", font="mono", name_size=50,
                  back=[("glow_spot", dict(cx=680, cy=60, r=260, op=0.3)),
                        ("speckle", dict(n=70, op=0.3, rmax=2.2)),
                        ("pixels", dict(size=10))]),
      header=dict(style="rule", font="mono", case="upper", ornament="block"),
    ...
    T("granite", "Granite", "elemental", "Rock strata and grain. The only theme with no motion at all.",
      ...
      light=dict(bg="#eeeff2", bg2="#d3d6dd", ink="#1b1d22", muted="#5b606b", acc="#3a3f4b", acc2="#7d8494"),
        hero=dict(type="plain", font="sans", ls=1,
                  back=[("strata", dict(bands=9, jag=7)),
                        ("lava", dict(cracks=2, dark=dict(colour="#c8963c"),
                                      light=dict(colour="#96550a")))],
    ...
    T("anamorphic", "Anamorphic", "gritty", "Letterbox bars and a horizontal lens flare. Cinema.",
      ...
      light=dict(bg="#f3f1ec", bg2="#d5d0c4", ink="#15181f", muted="#5c6270", acc="#1f4fa8", acc2="#3f7fdf"),
        hero=dict(type="plain", font="sans", ls=3, name_size=46,
                  back=[("stars", dict(n=70, dark=dict(), light=dict(skip=True))),
                        ("dust", dict(n=30, op=0.25)),
                        ("flare", dict(yy=96)),
                        ("flare", dict(yy=122))],
                  front=[("vignette", dict(strength=0.5)),
                         ("letterbox", dict(bar=26, dark=dict(colour="#000000"),
                                            light=dict(colour="#15181f")))]),
...
# Per-theme body art direction, applied on top of the family and register defaults the
# engine derives. This is the file where a look is DESIGNED, so the overrides live here
# rather than in the drawing code: a theme that wants a terminal window for its trust
# box, a hazard strip framing its page, or different weather behind its body says so on
# its own line. Keys:
#   note     "terminal" draws the trust callout as a window with title-bar lights
#   edge     "hazard" | "rivets" | "letterbox" frames the page (top of the first slice,
#            bottom of the last)
#   ambient  (motif, kwargs) to override the weather resolved from the hero
#   ambient_op  opacity override for the weather layer
BODY = {
    # terminal-voiced tech: the callout is a window on these, not a card
    "operator":   dict(note="terminal"),
    "crt":        dict(note="terminal"),
    "matrix":     dict(note="terminal", ambient=("datarain", dict(cols=16)), ambient_op=0.08),
    "mainframe":  dict(note="terminal", ambient=("punchcard", dict(cols=26, rows=40, op=0.5))),
    "cyberpunk":  dict(note="terminal"),
    "datacenter": dict(note="terminal"),
    # armoured and industrial: the page wears its warning strip
    "war":        dict(edge="hazard"),
    "toxic":      dict(edge="hazard"),
    "bunker":     dict(edge="hazard"),
    "mech":       dict(edge="hazard"),
    "steampunk":  dict(edge="rivets"),
    "survival":   dict(edge="rivets"),
    "wasteland":  dict(ambient=("scratches", dict(n=10)), ambient_op=0.2),
    # cinema and shadow
    "anamorphic": dict(edge="letterbox"),
    "noir":       dict(ambient=("blinds", dict(n=6)), ambient_op=0.06),
    # material worlds whose ground IS the theme
    "timber":     dict(ambient=("woodgrain", dict(lines=26, knots=3)), ambient_op=0.1),
    "granite":    dict(ambient=("strata", dict(bands=16, jag=4)), ambient_op=0.05),
    "western":    dict(ambient=("dust", dict(n=50))),
    "vaporwave":  dict(ambient=("halftone", dict(spacing=12))),
    "ledger":     dict(ambient=("blocks", dict(n=8, size=34)), ambient_op=0.07),
    "circuit":    dict(ambient=("traces", dict(n=8, draw=False)), ambient_op=0.07),
    "ocean":      dict(ambient=("bubbles", dict(n=16))),
    "abyss":      dict(ambient=("bubbles", dict(n=12)), ambient_op=0.1),
    "forest":     dict(ambient=("fireflies", dict(n=10)), ambient_op=0.5),
    "aurora":     dict(ambient=("stars", dict(n=60))),
}
